...
#include<cctype>  // for testing and manipulating of characters
#include<iomanip> // It provides functionality for setting various properties of the input/output 
...
int main()
{
	...
	cout<<"\n\t\t!!!!!!!!!!!!!!!!!!!!!!!!!!!!";
		...
		cin.ignore();
		cin.get();
    }while(ch!='8');  // it runs the code until user press 8
	return 0;
}
...
void write_Bank_Account()
{
	Bank_Account ac;
	ofstream outFile;
	outFile.open("Bank_Account.dat",ios::binary|ios::app);  // bank account details are stored in a bianary file
	ac.create_Bank_Account();
	outFile.write(reinterpret_cast<char *> (&ac), sizeof(Bank_Account));
	outFile.close();
}

// it opens bank_account_dat in binary mode for reading if the file can't be opened it gives error msg
// It also opens a temporary file named Temp.dat in binary mode for writing.
...
void display_all()
{
	system("CLS"); // clears the console screen and displays all the bank account holders
	Bank_Account ac;
	ifstream inFile;
	inFile.open("Bank_Account.dat",ios::binary);
	if(!inFile)
